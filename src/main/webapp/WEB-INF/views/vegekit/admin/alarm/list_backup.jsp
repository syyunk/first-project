	...
	<table>
		<tr>
			<td colspan="18" align="right">
				<button onclick="window.location='/vegekit/admin/alarm/writeForm'">알림등록</button>
			</td>
		</tr>
		<tr>
			<td>No.</td>
		...
		<c:forEach var="list" items="${list}">
			<tr>
				<td>${list.anum}</td>
				<td>${list.atype}</td>
				<td><a class="move" href="${list.anum}">${list.acnt}</a></td>
				<td>${list.ardate}</td>
				<td>${list.is_read}</td>
				<td>${list.mid}</td>
				<td>
					...
					<button class="deleteBtn" href="${list.anum}">삭제</button>
				</td>
			</tr>
		</c:forEach>
	</table>
	...
	<script>
		$(document).ready(function(){
			...
			$(".move").click(function(e){
				...
				pagingForm.find("input[name='anum']").val($(this).attr("href"));
				console.log(pagingForm.find("input[name='anum']").val());
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/alarm/detail"); 
				pagingForm.submit();  // 서브밋! 이동!!
			});
