	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
		...
		*/
		...
		if(session != null) {
			...
			if(redirectUrl != null) {
			         if(redirectUrl.equals("http://localhost:8080/vegekit/shop/subcart") 
			         || redirectUrl.equals("http://localhost:8080/vegekit/common/resetPwPro")) {
			        	 session.removeAttribute("prevPage");
			        	 response.sendRedirect("/vegekit/main");
			         }else {
				         System.out.println("redirecturl!!!!!!!!!!: "+redirectUrl);
				         session.removeAttribute("prevPage");
